...
def apaccompcompscnd(request):
    comppmscnd = Accomplishment.objects.filter(bapmonth__icontains='ነሃሴ').values('erabudget__bprojectname','budgetedap__bapinBr','budgetedap__bapinKm','actionInBr','actionInKm').annotate(compl=(F('actionInKm')/F('budgetedap__bapinKm'))*100, compf=(F('actionInBr')/F('budgetedap__bapinBr'))*100).order_by('id')
    
    #Q(bapmonth__icontains='ሐምሌ')|Q(bapmonth__icontains='ነሃሴ')
    apall = BudgetedAP.objects.all()
    apacompall = apall.filter(Q(month=1)|Q(month=2)).values('erabudget').annotate(aptotalbr=Sum('bapinBr'),acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    result = ERABudget.objects.filter(Q(accomplishment__bapmonth='ሐምሌ')|Q(accomplishment__bapmonth='ነሃሴ')|Q(budgetedap__month=1)|Q(budgetedap__month=2)).values('bprojectname').annotate(fincomp=Sum('accomplishment__actionInBr')/Sum('budgetedap__bapinBr')*100, phycomp=Sum('accomplishment__actionInKm')/Sum('budgetedap__bapinKm')*100).order_by('id')
    
    zipscnd = zip(comppmscnd,apacompall)
    
    result3 = ERABudget.objects.filter(Q(budgetedap__month=1)|Q(budgetedap__month=2)|Q(budgetedap__month=3)).values('bprojectname').annotate(total_br=Sum('budgetedap__bapinBr')).order_by('id')

    combined_queryset = list(chain(result, result3))
    #result_list = result.intersection(result3)
    
    
    #, accompt_br=Sum('accomplishment__actionInBr'), scndmcompbr=(F('accompt_br')/F('apt_br'))*100
    # intersection method
    #result_list = page_list.intersection(article_list, post_list)
    #BudgetedAP.objects.filter(Q(month__icontains='1')|Q(month__icontains='2')).values('erabudget__bprojectname').annotate(total=Sum('bapinBr'),compl=(F('accomplishment__actionInKm')/F('bapinKm'))*100, compf=(F('accomplishment__actionInBr')/F('bapinBr'))*100).order_by('id')
 
    #tbr=F('bapinBr') + F('accomplishment__actionInBr')

    return render(request, 'rasmApp/apaccomp_compscnd.html', {'comppmscnd':comppmscnd, 'result':result, 'result3':result3, 'combined_queryset':combined_queryset, 'zipscnd':zipscnd})#, 'result_list':result_list})

def apaccompcompthrd(request):
    comppmthrd = Accomplishment.objects.filter(bapmonth__icontains='መስከረም').values('erabudget__bprojectname','budgetedap__bapinBr','budgetedap__bapinKm','actionInBr','actionInKm').annotate(compl=(F('actionInKm')/F('budgetedap__bapinKm'))*100, compf=(F('actionInBr')/F('budgetedap__bapinBr'))*100).order_by('id')
    
    apall = BudgetedAP.objects.all()
    apacompall = apall.filter(Q(month=1)|Q(month=2)|Q(month=3)).values('erabudget').annotate(aptotalbr=Sum('bapinBr'),acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)

    zipthrd = zip(comppmthrd,apacompall)
    
    return render(request, 'rasmApp/apaccomp_compthrd.html', {'zipthrd':zipthrd})

def apaccompcompfrth(request):
    comppmfrth = Accomplishment.objects.filter(bapmonth__icontains='ጥቅምት').values('erabudget__bprojectname','budgetedap__bapinBr','budgetedap__bapinKm','actionInBr','actionInKm').annotate(total=Sum('budgetedap__bapinBr'),compl=(F('actionInKm')/F('budgetedap__bapinKm'))*100, compf=(F('actionInBr')/F('budgetedap__bapinBr'))*100).order_by('id')
    
    apall = BudgetedAP.objects.all()
    apacompall = apall.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)).values('erabudget').annotate(aptotalbr=Sum('bapinBr'),acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)

    zipfrth = zip(comppmfrth,apacompall)
    
    return render(request, 'rasmApp/apaccomp_compfrth.html', {'zipfrth':zipfrth})

def apaccompcompffth(request):
    comppmffth = Accomplishment.objects.filter(bapmonth__icontains='ህዳር').values('erabudget__bprojectname','budgetedap__bapinBr','budgetedap__bapinKm','actionInBr','actionInKm').annotate(total=Sum('budgetedap__bapinBr'),compl=(F('actionInKm')/F('budgetedap__bapinKm'))*100, compf=(F('actionInBr')/F('budgetedap__bapinBr'))*100).order_by('id')
    
    apall = BudgetedAP.objects.all()
    apacompall = apall.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)).values('erabudget').annotate(aptotalbr=Sum('bapinBr'),acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)

    zipffth = zip(comppmffth,apacompall)
    
    return render(request, 'rasmApp/apaccomp_compffth.html', {'zipffth':zipffth})

def apaccompcompsixth(request):
    comppmsixth = Accomplishment.objects.filter(bapmonth__icontains='ታህሳስ').values('erabudget__bprojectname','budgetedap__bapinBr','budgetedap__bapinKm','actionInBr','actionInKm').annotate(total=Sum('budgetedap__bapinBr'),compl=(F('actionInKm')/F('budgetedap__bapinKm'))*100, compf=(F('actionInBr')/F('budgetedap__bapinBr'))*100).order_by('id')
    
    apall = BudgetedAP.objects.all()
    apacompall = apall.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)).values('erabudget').annotate(aptotalbr=Sum('bapinBr'),acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)

    zipsixth = zip(comppmsixth,apacompall)
    
    return render(request, 'rasmApp/apaccomp_compsixth.html', {'zipsixth':zipsixth})

def apaccompcompsevnth(request):
    comppmsevnth = Accomplishment.objects.filter(bapmonth__icontains='ጥር').values('erabudget__bprojectname','budgetedap__bapinBr','budgetedap__bapinKm','actionInBr','actionInKm').annotate(total=Sum('budgetedap__bapinBr'),compl=(F('actionInKm')/F('budgetedap__bapinKm'))*100, compf=(F('actionInBr')/F('budgetedap__bapinBr'))*100).order_by('id')
    
    apall = BudgetedAP.objects.all()
    apacompall = apall.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)).values('erabudget').annotate(aptotalbr=Sum('bapinBr'),acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)

    zipsevnth = zip(comppmsevnth,apacompall)
    
    return render(request, 'rasmApp/apaccomp_compsevnth.html', {'zipsevnth':zipsevnth})

def apaccompcompeith(request):
    comppmeith = Accomplishment.objects.filter(bapmonth__icontains='የካቲት').values('erabudget__bprojectname','budgetedap__bapinBr','budgetedap__bapinKm','actionInBr','actionInKm').annotate(total=Sum('budgetedap__bapinBr'),compl=(F('actionInKm')/F('budgetedap__bapinKm'))*100, compf=(F('actionInBr')/F('budgetedap__bapinBr'))*100).order_by('id')
    
    apall = BudgetedAP.objects.all()
    apacompall = apall.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)).values('erabudget').annotate(aptotalbr=Sum('bapinBr'),acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)

    zipeith = zip(comppmeith,apacompall)
    
    return render(request, 'rasmApp/apaccomp_compeith.html', {'zipeith':zipeith})

def apaccompcompninth(request):
    comppmninth = Accomplishment.objects.filter(bapmonth__icontains='መጋቢት').values('erabudget__bprojectname','budgetedap__bapinBr','budgetedap__bapinKm','actionInBr','actionInKm').annotate(total=Sum('budgetedap__bapinBr'),compl=(F('actionInKm')/F('budgetedap__bapinKm'))*100, compf=(F('actionInBr')/F('budgetedap__bapinBr'))*100).order_by('id')
    
    apall = BudgetedAP.objects.all()
    apacompall = apall.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)).values('erabudget').annotate(aptotalbr=Sum('bapinBr'),acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)

    zipninth = zip(comppmninth,apacompall)
    
    return render(request, 'rasmApp/apaccomp_compninth.html', {'zipninth':zipninth})

def apaccompcomptenth(request):
    comppmtenth = Accomplishment.objects.filter(bapmonth__icontains='ሚያዚያ').values('erabudget__bprojectname','budgetedap__bapinBr','budgetedap__bapinKm','actionInBr','actionInKm').annotate(total=Sum('budgetedap__bapinBr'),compl=(F('actionInKm')/F('budgetedap__bapinKm'))*100, compf=(F('actionInBr')/F('budgetedap__bapinBr'))*100).order_by('id')
    
    apall = BudgetedAP.objects.all()
    apacompall = apall.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)|Q(month=10)).values('erabudget').annotate(aptotalbr=Sum('bapinBr'),acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'),fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)

    ziptenth = zip(comppmtenth,apacompall)
    
    return render(request, 'rasmApp/apaccomp_comptenth.html', {'ziptenth':ziptenth})

def apaccompcompelvnth(request):   
    projcomppmelvnth = BudgetedAP.objects.filter(month=11).values('erabudget__bprojectname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    projapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)|Q(month=10)|Q(month=11)).values('erabudget__bprojectname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)

    projelevenzip = zip(projcomppmelvnth,projapacompall)
    
    projaptpmbrsum = projcomppmelvnth.aggregate(projaptpmbrsum=Sum('aptotalpmonthbr'))['projaptpmbrsum']
    projacomptpmbrsum = projcomppmelvnth.aggregate(projacomptpmbrsum=Sum('acomptotalpmonthbr'))['projacomptpmbrsum']
    projaptpmkmsum = projcomppmelvnth.aggregate(projaptpmkmsum=Sum('aptotalpmonthkm'))['projaptpmkmsum']
    projacomptpmkmsum = projcomppmelvnth.aggregate(projacomptpmkmsum=Sum('acomptotalpmonthkm'))['projacomptpmkmsum']
    projfincomptpmsum = projcomppmelvnth.aggregate(projfincomptpmsum=Sum('fincomppmonth'))['projfincomptpmsum']
    projphycomptpmsum = projcomppmelvnth.aggregate(projphycomptpmsum=Sum('phycomppmonth'))['projphycomptpmsum']
    ...
    return render(request, 'rasmApp/apaccomp_compelvnth.html', {'projelevenzip':projelevenzip, 'projaptpmbrsum':projaptpmbrsum, 'projacomptpmbrsum':projacomptpmbrsum, 'projaptpmkmsum':projaptpmkmsum, 'projacomptpmkmsum':projacomptpmkmsum, 'projfincomptpmsum':projfincomptpmsum, 'projphycomptpmsum':projphycomptpmsum, 'projaptbrsum':projaptbrsum, 'projacomptbrsum':projacomptbrsum, 'projaptkmsum':projaptkmsum, 'projacomptkmsum':projacomptkmsum, 'projfincompsum':projfincompsum, 'projphycompsum':projphycompsum})

def apaccompcomplast(request):   
    projcomppmyr = BudgetedAP.objects.filter(month=12).values('erabudget__bprojectname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    projapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)|Q(month=10)|Q(month=11)|Q(month=12)).values('erabudget__bprojectname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)

    projyrzip = zip(projcomppmyr,projapacompall)
    
    projaptpmbrsum = projcomppmyr.aggregate(projaptpmbrsum=Sum('aptotalpmonthbr'))['projaptpmbrsum']
    projacomptpmbrsum = projcomppmyr.aggregate(projacomptpmbrsum=Sum('acomptotalpmonthbr'))['projacomptpmbrsum']
    projaptpmkmsum = projcomppmyr.aggregate(projaptpmkmsum=Sum('aptotalpmonthkm'))['projaptpmkmsum']
    projacomptpmkmsum = projcomppmyr.aggregate(projacomptpmkmsum=Sum('acomptotalpmonthkm'))['projacomptpmkmsum']
    projfincomptpmsum = projcomppmyr.aggregate(projfincomptpmsum=Sum('fincomppmonth'))['projfincomptpmsum']
    projphycomptpmsum = projcomppmyr.aggregate(projphycomptpmsum=Sum('phycomppmonth'))['projphycomptpmsum']
    projaptbrsum = projapacompall.aggregate(projaptbrsum=Sum('aptotalbr'))['projaptbrsum']
    projacomptbrsum = projapacompall.aggregate(projacomptbrsum=Sum('acomptotalbr'))['projacomptbrsum']
    projaptkmsum = projapacompall.aggregate(projaptkmsum=Sum('aptotalkm'))['projaptkmsum']
    projacomptkmsum = projapacompall.aggregate(projacomptkmsum=Sum('acomptotalkm'))['projacomptkmsum']
...
def district_compare_firstmonth(request):    
    discomppmfirst = BudgetedAP.objects.filter(month=1).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(month=1).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def district_compare_second(request):    
    discomppmsecond = BudgetedAP.objects.filter(month=2).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def district_compare_third(request):    
    discomppmthird = BudgetedAP.objects.filter(month=3).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def district_compare_fourth(request):    
    discomppmfourth = BudgetedAP.objects.filter(month=4).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def district_compare_fifth(request):    
    discomppmfifth = BudgetedAP.objects.filter(month=5).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def district_compare_sixth(request):    
    discomppmsixth = BudgetedAP.objects.filter(month=6).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def district_compare_seventh(request):    
    discomppmseventh = BudgetedAP.objects.filter(month=7).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def district_compare_nineth(request):    
    discomppmnineth = BudgetedAP.objects.filter(month=9).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'), acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def district_compare_tenth(request):    
    discomppmtenth = BudgetedAP.objects.filter(month=10).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)|Q(month=10)).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def district_compare_elvm(request):    
    discomppmelvm = BudgetedAP.objects.filter(month=11).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)|Q(month=10)|Q(month=11)).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def district_compare_yr(request):    
    discomppmyr = BudgetedAP.objects.filter(month=12).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)|Q(month=10)|Q(month=11)|Q(month=12)).values('erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def ownf_district_comp_first(request):    
    ownfdiscomppmfirst = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=1)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'), Q(month=1)).values('erabudget__bcontractor','erabudget__bdistrict__districtname').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'), acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    ...
    return render(request, 'rasmApp/ownf_dist_apacompcomp_firstm.html', {'ownfdisfirstmzip':ownfdisfirstmzip, 'ownfaptbrsum':ownfaptbrsum, 'ownfacomptbrsum':ownfacomptbrsum, 'ownfaptkmsum':ownfaptkmsum, 'ownfacomptkmsum':ownfacomptkmsum, 'ownf_fincompsum':ownf_fincompsum, 'ownf_phycompsum':ownf_phycompsum, 'ownfaptpmbrsum':ownfaptpmbrsum, 'ownfacomptpmbrsum':ownfacomptpmbrsum, 'ownfaptpmkmsum':ownfaptpmkmsum, 'ownfacomptpmkmsum':ownfacomptpmkmsum, 'ownf_fincomptpmsum':ownf_fincomptpmsum, 'ownf_phycomptpmsum':ownf_phycomptpmsum})

def ownf_district_comp_second(request):    
    ownfdiscomppmtwo = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=2)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def ownf_district_comp_third(request):    
    ownfdiscomppmthree = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=3)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def ownf_district_comp_fourth(request):    
    ownfdiscomppmfour = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=4)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def ownf_district_comp_fifth(request):    
    ownfdiscomppmfive = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=5)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def ownf_district_comp_sixth(request):    
    ownfdiscomppmsix = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=6)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def ownf_district_comp_seventh(request):    
    ownfdiscomppmseven = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=7)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def ownf_district_comp_nineth(request):    
    ownfdiscomppmnine = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=9)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def ownf_district_comp_tenth(request):    
    ownfdiscomppmten = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=10)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def ownf_district_comp(request):    
    ownfdiscomppmelvm = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=11)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def ownf_district_compyr(request):    
    ownfdiscompyr = BudgetedAP.objects.filter(Q(erabudget__bcontractor__icontains='የራስ ሀይል ተቋራጭ'),Q(month=12)).values('erabudget__bdistrict__districtname').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_firstm (request):    
    interventioncomppmfirstm = BudgetedAP.objects.filter(month=1).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(month=1).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    ...
    return render(request, 'rasmApp/intervention_apacompcomp_first.html', {'interventionfirstmzip':interventionfirstmzip, 'disapacompall':disapacompall,  'intervenaptbrsum':intervenaptbrsum, 'intervenacomptbrsum':intervenacomptbrsum, 'intervenaptkmsum':intervenaptkmsum, 'intervenacomptkmsum':intervenacomptkmsum, 'interven_fincompsum':interven_fincompsum, 'interven_phycompsum':interven_phycompsum, 'intervenaptpmbrsum':intervenaptpmbrsum, 'intervenacomptpmbrsum':intervenacomptpmbrsum, 'intervenaptpmkmsum':intervenaptpmkmsum, 'intervenacomptpmkmsum':intervenacomptpmkmsum, 'interven_fincomptpmsum':interven_fincomptpmsum, 'interven_phycomptpmsum':interven_phycomptpmsum})

def intervention_summary_secondm (request):    
    interventioncomppmsecondm = BudgetedAP.objects.filter(month=2).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_thirdm (request):    
    interventioncomppmthreem = BudgetedAP.objects.filter(month=3).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100).order_by('erabudget__bworktype__maintenancetype')
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_fourthm (request):    
    interventioncomppmfourm = BudgetedAP.objects.filter(month=4).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_fifthm (request):    
    interventioncomppmfivem = BudgetedAP.objects.filter(month=5).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_sixthm (request):    
    interventioncomppmsixm = BudgetedAP.objects.filter(month=6).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_seventhm (request):    
    interventioncomppmsevenm = BudgetedAP.objects.filter(month=7).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_eighthm (request):    
    interventioncomppmeightm = BudgetedAP.objects.filter(month=8).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_ninthm (request):    
    interventioncomppmninem = BudgetedAP.objects.filter(month=9).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_tenthm (request):    
    interventioncomppmtenm = BudgetedAP.objects.filter(month=10).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)|Q(month=10)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_eleventhm (request):    
    interventioncomppmelvm = BudgetedAP.objects.filter(month=11).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)|Q(month=10)|Q(month=11)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'), acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def intervention_summary_yr (request):    
    interventioncomppmyr = BudgetedAP.objects.filter(month=12).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ')| Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalpmonthbr = Sum('bapinBr'), acomptotalpmonthbr = Sum('accomplishment__actionInBr'), aptotalpmonthkm = Sum('bapinKm'), acomptotalpmonthkm = Sum('accomplishment__actionInKm'), fincomppmonth=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomppmonth=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
    
    disapacompall = BudgetedAP.objects.filter(Q(month=1)|Q(month=2)|Q(month=3)|Q(month=4)|Q(month=5)|Q(month=6)|Q(month=7)|Q(month=8)|Q(month=9)|Q(month=10)|Q(month=11)|Q(month=12)).exclude(Q(erabudget__bworktype__maintenancetype='የካሳ ክፍያ') | Q(erabudget__bworktype__maintenancetype='ኤርጎኖሚክስ')| Q(erabudget__bworktype__maintenancetype='የመንገድ ደህንነት ስራ')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ ስራዎች')| Q(erabudget__bworktype__maintenancetype='የማማከር አገልግሎት')| Q(erabudget__bworktype__maintenancetype='የሚዛን ጣቢያ የማዘመን ስራዎች')| Q(erabudget__bworktype__maintenancetype='ድልድይ መልሶ ግንባታ')| Q(erabudget__bworktype__maintenancetype='ድልድይ/ፉካ ጥገና ስራ')).values('erabudget__bworktype__maintenancetype').annotate(aptotalbr=Sum('bapinBr'),acomptotalbr=Sum('accomplishment__actionInBr'), aptotalkm=Sum('bapinKm'),acomptotalkm=Sum('accomplishment__actionInKm'), fincomp=(Sum('accomplishment__actionInBr')/Sum('bapinBr'))*100, phycomp=(Sum('accomplishment__actionInKm')/Sum('bapinKm'))*100)
...
def road_segment_ext(request, pk):
    context = {}
    roadsegments = RoadSegment.objects.get(id=pk)
    roadsegmentext = roadsegments.roadsegmentext_set.all()
    roadsegmentext_count = roadsegmentext.count()
    
    context={'roadsegments':roadsegments, 'roadsegmentext':roadsegmentext, 'roadsegmentext_count':roadsegmentext_count}
...
#def create_condition_survey(request):
    #RoadConditionSurveyFormSet = modelformset_factory(RoadConditionSurvey, fields=('roadsegext','problem','severity', 'extent', 'activity', 'qty'), extra=5)
    #return render(request, 'rasmApp/condition_survey_form.html', {'form':form})


def create_condition_survey(request):
    context = {}
    form = RoadConditionSurveyForm()
    condsurvey = RoadConditionSurvey.objects.all()
    context['condsurvey'] = condsurvey
    context['title'] = 'RoadConditionSurvey'
    if request.method == 'POST':
        if 'save' in request.POST:
            pk = request.POST.get('save')
            if not pk:
                form = RoadConditionSurveyForm(request.POST)
            else:
                conds = RoadConditionSurvey.objects.get(id=pk)
                form = RoadConditionSurveyForm(request.POST, instance=conds)
            form.save()
            form = RoadConditionSurveyForm()
        elif 'delete' in request.POST:
            pk = request.POST.get('delete')
            ...
            pk = request.POST.get('edit')
            conds = RoadConditionSurvey.objects.get(id=pk)
            form = RoadConditionSurveyForm(instance=conds)
    context['form'] = form
